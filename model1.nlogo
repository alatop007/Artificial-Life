;; This program adapted the termite inbuilt model in Netlogo
...
to setup
  ...
  ask patches
  [
    ;; randomly distribute green wood chips
    if random-float 100 < GreenDensity [ set pcolor green ]
    ;; randomly distribute blue wood chips
    if random-float 100 < BlueDensity [ set pcolor blue ]

    ;; Create two round objects with the as sliders parameters
...
to search-for-chip
  ifelse pcolor = green    ;; If chip is green

  [ set pcolor black       ;; Pick it up
    set shape "arrow"      ;; change shape to arrow
    fd 20                  ;; Move forward
  ]

  [ ifelse pcolor = blue      ;; If chip is blue
    [
       set pcolor black       ;; Pick it up
       set shape "triangle"   ;; change shape to triangle
       fd 20
    ]
...
to find-new-pile
  if pcolor != green and pcolor != blue   ;; Keep looking for green and blue chips
...
to put-down-chip
  ifelse pcolor = black
  [
      if shape = "arrow" [set pcolor blue]        ;; drop blue chips in empty space
      if shape = "triangle" [set pcolor green]    ;; drop green chips in empty space
      get-away
  ]
